...
def event_consumer(host_kafka, host_zookeeper, topic, assertion_event_content, match_partially=False, timeout_sec=20):
    try:
        ...
        consumer = topic.get_balanced_consumer(
            consumer_group='Group-Lib_Kafka',
        ...
        timeout = time.time() + int(timeout_sec)
        got_msg = False
        while True:
...
def event_consume_to_latest(host_kafka, host_zookeeper, topic, timeout):
    try:
        client = KafkaClient(hosts=host_kafka, socket_timeout_ms=10000)
        topic = client.topics[str(topic)]
        ...
        Nodata = True
        return_result = []
        return_result.append('StartConsume')
        ...
        while True:
            ...
            if msg is not None:
                Nodata = False
                msg_print('Got event:' + str(msg.offset) + ' - ' + msg.value)
                return_result.append(msg.value)
                return_result[0] = 'EventProcessing'
            if time.time() > timeout:
                break

        consumer.stop()
        if Nodata:
            return_result[0] = 'Nodata'
        else:
            return_result[0] = 'DataGet'
    except exceptions.KafkaException:
        return_result[0] = 'KafkaConnectFailed'

    msg_print('Consumer stopped! ' + str(return_result[0]))
    return return_result
...
def event_commit_to_latest(host_kafka, host_zookeeper, topic):
    try:
        client = KafkaClient(hosts=host_kafka, socket_timeout_ms=10000)
        topic = client.topics[str(topic)]
...
def event_commit_to_earliest(host_kafka, host_zookeeper, topic):
    try:
        client = KafkaClient(hosts=host_kafka, socket_timeout_ms=10000)
        topic = client.topics[str(topic)]
